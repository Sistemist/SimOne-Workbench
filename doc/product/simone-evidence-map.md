...
| SIM Coach explains scanner results, distinguishes Coach/Wiki/control plane, shows the operating role stack, and translates inherited Workbench terms | `ui/src/pages/SimCoach.tsx`, `ui/src/pages/SimCoach.test.tsx`, scanner handoff in `ui/src/pages/SystemsBottleneckScanner.tsx`, PRD | Implemented and tested | More contextual Coach entry points beyond scanner |
...
| Route decisions have an auditable ledger with post-run review fields, cost links, run-detail links, output artifact links, lane-specific evidence prompts, initial route-boundary evals, and an operator audit/review surface | `packages/db/src/schema/model_route_decisions.ts`, `packages/db/src/schema/cost_events.ts`, `packages/db/src/migrations/0127_model_route_decision_review_fields.sql`, `packages/db/src/migrations/0128_cost_event_model_route_decision.sql`, `server/src/routes/costs.ts`, `server/src/services/model-route-decisions.ts`, `ui/src/pages/ModelRoutingAudit.tsx`, `ui/src/api/modelRouting.ts`, `packages/shared/src/validators/model-route-decision.ts`, `packages/shared/src/validators/cost.ts`, `server/src/__tests__/model-route-decisions-routes.test.ts`, `ui/src/pages/ModelRoutingAudit.test.tsx`, `ui/src/components/CompanySettingsSidebar.test.tsx`, `evals/promptfoo/tests/model-routing.yaml` | Implemented and tested | Link decisions to actual provider execution and expand evaluations |
