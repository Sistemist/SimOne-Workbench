...
| Route decisions have an auditable ledger with post-run review fields | `packages/db/src/schema/model_route_decisions.ts`, `packages/db/src/migrations/0127_model_route_decision_review_fields.sql`, `server/src/routes/costs.ts`, `packages/shared/src/validators/model-route-decision.ts`, `server/src/__tests__/model-route-decisions-routes.test.ts` | Implemented and tested | Link decisions to actual provider execution, run records, cost events, and output artifacts |
