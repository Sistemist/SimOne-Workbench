...
| Route decisions have an auditable ledger with post-run review fields and cost links | `packages/db/src/schema/model_route_decisions.ts`, `packages/db/src/schema/cost_events.ts`, `packages/db/src/migrations/0127_model_route_decision_review_fields.sql`, `packages/db/src/migrations/0128_cost_event_model_route_decision.sql`, `server/src/routes/costs.ts`, `packages/shared/src/validators/model-route-decision.ts`, `packages/shared/src/validators/cost.ts`, `server/src/__tests__/model-route-decisions-routes.test.ts` | Implemented and tested | Link decisions to actual provider execution, run records, and output artifacts |
