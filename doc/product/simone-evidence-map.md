...
| Sprint Zero Brief carries into SIM Starter, first-map review, bounded child tasks, and reviewed map artifact records | `ui/src/components/OnboardingWizard.tsx`, `ui/src/components/OnboardingWizard.simone-starter.test.tsx`, `ui/src/components/SimOneSprintZeroGuide.tsx`, `ui/src/components/SimOneSprintZeroGuide.test.tsx`, `ui/src/pages/IssueDetail.tsx`, `ui/src/pages/IssueDetail.test.tsx` | Implemented and tested | Persist selected reviewed map content directly into SIM Wiki pages; polish artifact into stronger public share package |
...
| Route decisions have an auditable ledger with post-run review fields, cost links, run-detail links, output artifact links, and an operator audit/review surface | `packages/db/src/schema/model_route_decisions.ts`, `packages/db/src/schema/cost_events.ts`, `packages/db/src/migrations/0127_model_route_decision_review_fields.sql`, `packages/db/src/migrations/0128_cost_event_model_route_decision.sql`, `server/src/routes/costs.ts`, `server/src/services/model-route-decisions.ts`, `ui/src/pages/ModelRoutingAudit.tsx`, `ui/src/api/modelRouting.ts`, `packages/shared/src/validators/model-route-decision.ts`, `packages/shared/src/validators/cost.ts`, `server/src/__tests__/model-route-decisions-routes.test.ts`, `ui/src/pages/ModelRoutingAudit.test.tsx`, `ui/src/components/CompanySettingsSidebar.test.tsx` | Implemented and tested | Link decisions to actual provider execution and evaluations |
