...
`;
  ...
  it("bootstraps required local wiki files through the local folder API", async () => {
    ...
    expect(writes.find((write) => write.path === "wiki/concepts/simone-evidence-map.md")?.contents).toContain("state ownership contract");
    ...
    expect(writes.find((write) => write.path === "wiki/concepts/simone-docs-index.md")?.contents).toContain("Where does state live");
    ...
    expect(writes.find((write) => write.path === "wiki/concepts/simone-prd.md")?.contents).toContain("State Ownership Contract");
    expect(writes.find((write) => write.path === "wiki/concepts/simone-prd.md")?.contents).toContain("If something should be trusted later, it needs a durable owner and a receipt.");
    ...
    expect(writes.find((write) => write.path === "wiki/concepts/simone-agent-flows.md")?.contents).toContain("State Promotion");
    expect(writes.find((write) => write.path === "wiki/concepts/simone-agent-flows.md")?.contents).toContain("promote it to SIM Wiki when it should become durable knowledge");
